...
optimizer = torch.optim.Adam(two_tower.parameters(), lr=0.00001)
...
for i in range(len(qry_tensor)):
    ...
    pos_loss = criterion(qry_emb, pos_emb, torch.tensor([1.0], device=device))
    neg_loss = criterion(qry_emb, neg_emb, torch.tensor([-1.0], device=device))
...
model_path = 'two_tower_model_gpu.pth'
