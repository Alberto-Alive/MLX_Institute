...
with open('preprocessed_text8_corpus.pkl', 'wb') as f:
...
with open('text8_words_to_ids.json', 'w') as f:
    json.dump(words_to_ids, f)

# Save ids_to_words (index to word)
with open('text8_ids_to_words.json', 'w') as f:
...
for epoch in range(10):
  print("Time", epoch)
  wins = more_itertools.windowed(tokens[:1000], 3)
  prgs = tqdm.tqdm(enumerate(wins), total=len(tokens[:1000]), desc=f"Epoch {epoch+1}", leave=False)
  for i, tks in prgs:
