...
for epoch in range(10000):  # Set a more realistic epoch limit
    total_loss = 0
    correct_predictions = 0
    total_masked_tokens = 0
    
    with tqdm(total=len(loaded_training_data), desc=f"Epoch {epoch + 1}") as pbar:
        for masked_sequence, target_tokens in loaded_training_data[:100]:
            optimizer.zero_grad()

            # Convert masked_sequence to tensor and add batch dimension
            inputs = torch.tensor(masked_sequence).unsqueeze(0)  # Shape: [1, seq_length]
            outputs = B(inputs)  # Model's predicted logits for each token in the sequence
            ...
            for position, target_token_id in target_tokens:
                ...
                predicted_logits = outputs[0, position, :]  # Shape: [vocab_size]
...
#     # Outer tqdm progress bar for epochs
#     print(f"Epoch {epoch + 1}")
#     with tqdm(total=len(loaded_training_data), desc=f"Training Epoch {epoch + 1}") as pbar:
#         for masked_sequence, target_tokens in loaded_training_data:
#             optimizer.zero_grad()
            
#             # Convert input sequence to tensor
#             inputs = torch.tensor(masked_sequence).unsqueeze(0)
#             if inputs.max().item() >= vocab_size:
#                 raise ValueError("Input token exceeds vocabulary size.")

#             outputs = B(inputs)
            
#             # Initialize loss for this sample
#             sample_loss = 0
#             all_correct = True  # Flag to track if all predictions in this sequence are correct

#             for position, target_token in target_tokens:
#                 prediction = outputs[0, position, :]  # Predicted logits for the masked position
#                 target = torch.tensor([target_token])
#                 loss = F.nll_loss(prediction.unsqueeze(0), target)
#                 sample_loss += loss

#                 # Get predicted token and check if it matches the target
#                 predicted_token_id = prediction.argmax(dim=-1).item()
#                 predicted_token = idx_to_vocab[predicted_token_id]
#                 target_token_str = idx_to_vocab[target_token]
                
#                 print(f"Predicted token for position {position} (target: {target_token_str}): {predicted_token}")

#                 # Check if prediction is correct
#                 if predicted_token == target_token_str:
#                     correct_count += 1
#                 else:
#                     all_correct = False
...
#             # Update progress bar after each batch (sequence)
#             pbar.update(1)
...
#     if correct_count == len(loaded_training_data):
#         stop_training = True
#         print(f"Model correctly predicted all targets in this epoch. Stopping training.")
#         break
...
# torch.save(B.state_dict(), "./models/bert_model.pt")
# print("Model saved to ./models/bert_model.pt")
...
# Training loop
# stop_training = False
# for epoch in range(15):  # For simplicity, using fewer epochs; adjust as needed
#     total_loss = 0
#     random.shuffle(loaded_training_data)  # Shuffle training data each epoch
#     for masked_sequence, target_tokens in loaded_training_data:
#         optimizer.zero_grad()
...
#         # Calculate loss only at masked positions
#         loss = 0
#         for position, target_token in target_tokens:
#             prediction = outputs[0, position, :]  # Predicted logits for the masked position
#             target = torch.tensor([target_token])
...
#     print(f"Epoch {epoch + 1}: Average Loss: {total_loss / len(loaded_training_data)}")

    # Optionally, implement early stopping or a stopping criterion based on the loss value
