...
def generate_masked_samples(text, tokenizer, max_seq_length=128, num_masks=20):
    ...
    for i in range(0, len(tokens), max_seq_length):
        ...
        # Ensure only up to the max sequence length is considered
        masked_sequence = sequence[:]
        target_tokens = []

        # Select exactly `num_masks` positions randomly within the sequence
        # (exclude padding positions if present by limiting range to the actual sequence length)
        mask_positions = random.sample(range(128), num_masks)
        
        # Apply masking to the selected positions
        for pos in mask_positions:
            target_tokens.append((pos, masked_sequence[pos]))  # Save original token
            masked_sequence[pos] = MASK_TOKEN  # Mask the token
