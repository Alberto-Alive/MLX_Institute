...
class ENCODER(nn.Module):
    def __init__(self, embedding_dim=64, num_heads=8, num_patches=196, patch_embs_C_H_W=768):
        """
        Transformer Encoder that includes image patch embedding and multiple Magic layers.
        Args:
            embedding_dim (int): Dimension of embeddings.
            num_heads (int): Number of attention heads.
            num_patches (int): Maximum number of patches (sequence length).
        """
        super(ENCODER, self).__init__()
        self.num_patches = num_patches
        self.embedding_dim = embedding_dim
        self.num_heads = num_heads
        self.patch_embs_C_H_W = patch_embs_C_H_W
        # Image Patch Embedding: Converts flattened patches to embedding vectors
        self.image_embedding = nn.Linear(self.patch_embs_C_H_W, self.embedding_dim)  # 3 channels, 8x8 pixels

        # Positional Encoding
        self.pos_encoding = get_positional_encoding(self.num_patches, self.embedding_dim)
    ...
    def forward(self, inputs):
        ...
        Args:
            inputs: Tensor of shape [batch_size, 196, 3x16x16=768]
        Returns:
            Tensor of shape [batch_size, 196, embedding_dim]
        """
        batch_size, num_patches, patch_dim = inputs.size()

        # Embed patches
        embeddings = self.image_embedding(inputs)  # [batch_size, num_patches, embedding_dim]

        # Add Positional Encoding
        pos_enc = self.pos_encoding_buffer[:num_patches, :].unsqueeze(0).repeat(batch_size, 1, 1).to(inputs.device)
        embeddings = embeddings + pos_enc   

        # Pass through Magic layers
        for magic in self.magics:
            embeddings = magic(embeddings)  # [batch_size, 196, embedding_dim]
