...
class Magic(torch.nn.Module):
    def __init__(self,  embedding_dim, num_heads):
        ...
        self.W_Q = torch.nn.Linear(self.embedding_dim, self.embedding_dim) # project embedding sequence on query 9x9
        self.W_K = torch.nn.Linear(self.embedding_dim, self.embedding_dim) # project embedding sequence on query keys 9x9
        self.W_V = torch.nn.Linear(self.embedding_dim, self.embedding_dim) # project embedding sequence on query values 9x9
        self.W_O = torch.nn.Linear(self.embedding_dim, self.embedding_dim) # project concatenated head outputs 9x9
    
        self.layer_norm = torch.nn.LayerNorm(self.embedding_dim)
    
    def forward(self, inputs):
        batch_size, sequence_length, _ = inputs.size()
        Q = self.W_Q(inputs) # embedding matrix times the weights matrix for queries 3x9
        K = self.W_K(inputs) # embedding matrix times the weights matrix for keys 3x9
        V = self.W_V(inputs) # embedding matrix times the weights matrix for values 3x9
        ...
        attn_scores = torch.matmul(Q, K.transpose(-2, -1)) / math.sqrt(self.head_dim) # 3x3
        attn_probs = F.softmax(attn_scores, dim=-1) # 3x3
        head_outputs = torch.matmul(attn_probs, V) # 3x9
        concat_output = head_outputs.transpose(1, 2).contiguous().view(batch_size, sequence_length, self.embedding_dim) # 3x9
        out = self.W_O(concat_output) # 9x9
