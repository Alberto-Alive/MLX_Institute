...
optimizer = torch.optim.Adam(B.parameters(), lr=0.0015)
#optimizer = torch.optim.RMSprop(B.parameters(), lr=0.001, alpha=0.11, eps=1e-08, weight_decay=0)
#optimizer = torch.optim.AdamW(B.parameters(), lr=0.001, betas=(0.2, 0.44), eps=1e-08, weight_decay=0.01)
...
for epoch in range(500):
    ...
    with tqdm(total=len(train_loader), desc=f"Epoch {epoch + 1}") as pbar:
        for inputs, target_positions, target_words in train_loader:
            ...
            del inputs, target_positions, target_words, outputs, masked_outputs, masked_targets, preds, mask
...
model_save_path = f"./bert_model_epoch_{epoch + 1}_loss_{avg_loss:.4f}_acc_{accuracy:.4%}_time_{exec_time:.4f}_batch_{batch_size}.pt"
